...
containers.forEach(container => {
	...
	input.addEventListener('focus', () => {
		if (!input.value) {
			...
			//Placeholder jump
			tl.to(placeholder, { top: -20, lef: 0, scale: 0.7, duration: 0.5, ease: 'Power2.easeOut' }, '<10%');
		}
	});
});
...
const tl2 = gsap.timeline({ defaults: { duration: 0.4, ease: 'Power2.easeOut' } });
...
button.addEventListener('click', e => {
	e.preventDefault();
	tl3.to('.contact-left, .contact-right', { y: 30, opacity: 0, pointerEvents: 'none' });
	tl3.to('form', { scale: 0.8 }, '<');
	tl3.fromTo('.submitted', { opacity: 0, y: 30 }, { opacity: 1, y: 0 }, '<80%');
	gsap.set('#hand', {transformOrigin: 'topo center'})
	gsap.fromTo('#hand', { rotation: 0, y: 0 }, { rotation: -10, y: 2, ease: 'elastic(3,0.3)', duration: 2.5 });
});
